...
class TransferController extends Controller
{
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('transfers-add')){
            ...
            return view('transfer.create', compact('lims_warehouse_list', 'from_warehouses', 'purchase_ref_numbers'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }

    // get products from select auto complete function
    ...
    public function searchMaterialProduct(Request $request){
        $id = $request->id;
        $material_product = ProductPurchase::select('products.code', 'products.name', 'product_batches.batch_no', 'product_purchases.remaining_qty')
                            ->join('products', 'products.id', 'product_purchases.product_id')
                            ->leftJoin('product_batches', 'product_batches.id', 'product_purchases.product_batch_id')
                            ->where([
                                ['purchase_id', $id],
                                ['remaining_qty', '>', 0]
                            ])->get();
